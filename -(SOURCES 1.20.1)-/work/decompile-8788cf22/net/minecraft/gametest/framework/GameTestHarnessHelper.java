...
public class GameTestHarnessHelper {
    ...
    public EntityPlayer makeMockServerPlayerInLevel() {
        EntityPlayer entityplayer = new EntityPlayer(this.getLevel().getServer(), this.getLevel(), new GameProfile(UUID.randomUUID(), "test-mock-player")) {
            @Override
            public boolean isSpectator() {
                return false;
            }
        ...
        this.getLevel().getServer().getPlayerList().placeNewPlayer(new NetworkManager(EnumProtocolDirection.SERVERBOUND), entityplayer);
        return entityplayer;
    }
