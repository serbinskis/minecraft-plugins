            ...
            if (collection1.size() == 1) {
                if (collection.size() == 1) {
                    commandlistenerwrapper.sendSuccess(() -> {
                        return IChatBaseComponent.translatable(commandadvancement_action.getKey() + ".one.to.one.success", ((Advancement) collection1.iterator().next()).getChatComponent(), ((EntityPlayer) collection.iterator().next()).getDisplayName());
                    }, true);
                } else {
                    commandlistenerwrapper.sendSuccess(() -> {
                        return IChatBaseComponent.translatable(commandadvancement_action.getKey() + ".one.to.many.success", ((Advancement) collection1.iterator().next()).getChatComponent(), collection.size());
                    }, true);
                }
            } else if (collection.size() == 1) {
                commandlistenerwrapper.sendSuccess(() -> {
    ...
    private static int performCriterion(CommandListenerWrapper commandlistenerwrapper, Collection<EntityPlayer> collection, CommandAdvancement.Action commandadvancement_action, Advancement advancement, String s) {
        ...
        if (!advancement.getCriteria().containsKey(s)) {
            ...
            if (i == 0) {
                ...
                if (collection.size() == 1) {
                    commandlistenerwrapper.sendSuccess(() -> {
                        return IChatBaseComponent.translatable(commandadvancement_action.getKey() + ".criterion.to.one.success", s, advancement.getChatComponent(), ((EntityPlayer) collection.iterator().next()).getDisplayName());
                    }, true);
                } else {
                    commandlistenerwrapper.sendSuccess(() -> {
                        return IChatBaseComponent.translatable(commandadvancement_action.getKey() + ".criterion.to.many.success", s, advancement.getChatComponent(), collection.size());
                    }, true);
                }
