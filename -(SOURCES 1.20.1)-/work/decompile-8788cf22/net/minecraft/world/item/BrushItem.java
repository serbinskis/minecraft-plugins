...
public class BrushItem extends Item {
    ...
    private static final double MAX_BRUSH_DISTANCE = Math.sqrt(PlayerConnection.MAX_INTERACTION_DISTANCE) - 1.0D;
    ...
    public void onUseTick(World world, EntityLiving entityliving, ItemStack itemstack, int i) {
        if (i >= 0 && entityliving instanceof EntityHuman) {
            ...
            MovingObjectPosition movingobjectposition = this.calculateHitResult(entityliving);
    ...
    private MovingObjectPosition calculateHitResult(EntityLiving entityliving) {
        return ProjectileHelper.getHitResultOnViewVector(entityliving, (entity) -> {
            return !entity.isSpectator() && entity.isPickable();
        }, BrushItem.MAX_BRUSH_DISTANCE);
    }

    public void spawnDustParticles(World world, MovingObjectPositionBlock movingobjectpositionblock, IBlockData iblockdata, Vec3D vec3d, EnumMainHand enummainhand) {
        double d0 = 3.0D;
