...
public abstract class FluidTypeFlowing extends FluidType {
    ...
    private boolean canHoldFluid(IBlockAccess iblockaccess, BlockPosition blockposition, IBlockData iblockdata, FluidType fluidtype) {
        ...
        return block instanceof IFluidContainer ? ((IFluidContainer) block).canPlaceLiquid(iblockaccess, blockposition, iblockdata, fluidtype) : (!(block instanceof BlockDoor) && !iblockdata.is(TagsBlock.SIGNS) && !iblockdata.is(Blocks.LADDER) && !iblockdata.is(Blocks.SUGAR_CANE) && !iblockdata.is(Blocks.BUBBLE_COLUMN) ? (!iblockdata.is(Blocks.NETHER_PORTAL) && !iblockdata.is(Blocks.END_PORTAL) && !iblockdata.is(Blocks.END_GATEWAY) && !iblockdata.is(Blocks.STRUCTURE_VOID) ? !iblockdata.blocksMotion() : false) : false);
    }
