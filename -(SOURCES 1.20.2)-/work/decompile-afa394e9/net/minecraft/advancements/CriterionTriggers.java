...
public class CriterionTriggers {
    ...
    private static final BiMap<MinecraftKey, CriterionTrigger<?>> CRITERIA = HashBiMap.create();
    ...
    private static <T extends CriterionTrigger<?>> T register(String s, T t0) {
        ...
        if (CriterionTriggers.CRITERIA.putIfAbsent(minecraftkey, t0) != null) {
            throw new IllegalArgumentException("Duplicate criterion id " + minecraftkey);
        } else {
            return t0;
        }
    }
    ...
    @Nullable
    public static MinecraftKey getId(CriterionTrigger<?> criteriontrigger) {
        return (MinecraftKey) CriterionTriggers.CRITERIA.inverse().get(criteriontrigger);
    }
