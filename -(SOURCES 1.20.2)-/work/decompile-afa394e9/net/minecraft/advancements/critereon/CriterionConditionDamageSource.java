...
public record CriterionConditionDamageSource(List<TagPredicate<DamageType>> tags, Optional<CriterionConditionEntity> directEntity, Optional<CriterionConditionEntity> sourceEntity) {
    ...
    public static Optional<CriterionConditionDamageSource> fromJson(@Nullable JsonElement jsonelement) {
        return jsonelement != null && !jsonelement.isJsonNull() ? Optional.of((CriterionConditionDamageSource) SystemUtils.getOrThrow(CriterionConditionDamageSource.CODEC.parse(JsonOps.INSTANCE, jsonelement), JsonParseException::new)) : Optional.empty();
    }

    public JsonElement serializeToJson() {
        return (JsonElement) SystemUtils.getOrThrow(CriterionConditionDamageSource.CODEC.encodeStart(JsonOps.INSTANCE, this), IllegalStateException::new);
    }
