...
public record CriterionConditionPlayer(CriterionConditionValue.IntegerRange level, Optional<EnumGamemode> gameType, List<CriterionConditionPlayer.e<?>> stats, Object2BooleanMap<MinecraftKey> recipes, Map<MinecraftKey, CriterionConditionPlayer.c> advancements, Optional<CriterionConditionEntity> lookingAt) implements EntitySubPredicate {

    public static final int LOOKING_AT_RANGE = 100;
    public static final MapCodec<CriterionConditionPlayer> CODEC = RecordCodecBuilder.mapCodec((instance) -> {
        return instance.group(ExtraCodecs.strictOptionalField(CriterionConditionValue.IntegerRange.CODEC, "level", CriterionConditionValue.IntegerRange.ANY).forGetter(CriterionConditionPlayer::level), EnumGamemode.CODEC.optionalFieldOf("gamemode").forGetter(CriterionConditionPlayer::gameType), ExtraCodecs.strictOptionalField(CriterionConditionPlayer.e.CODEC.listOf(), "stats", List.of()).forGetter(CriterionConditionPlayer::stats), ExtraCodecs.strictOptionalField(ExtraCodecs.object2BooleanMap(MinecraftKey.CODEC), "recipes", Object2BooleanMaps.emptyMap()).forGetter(CriterionConditionPlayer::recipes), ExtraCodecs.strictOptionalField(Codec.unboundedMap(MinecraftKey.CODEC, CriterionConditionPlayer.c.CODEC), "advancements", Map.of()).forGetter(CriterionConditionPlayer::advancements), ExtraCodecs.strictOptionalField(CriterionConditionEntity.CODEC, "looking_at").forGetter(CriterionConditionPlayer::lookingAt)).apply(instance, CriterionConditionPlayer::new);
    });
    ...
    public boolean matches(Entity entity, WorldServer worldserver, @Nullable Vec3D vec3d) {
        if (!(entity instanceof EntityPlayer)) {
            ...
            if (!this.level.matches(entityplayer.experienceLevel)) {
                return false;
            } else if (this.gameType.isPresent() && this.gameType.get() != entityplayer.gameMode.getGameModeForPlayer()) {
                return false;
            } else {
    ...
    private static record e<T> (StatisticWrapper<T> type, Holder<T> value, CriterionConditionValue.IntegerRange range, Supplier<Statistic<T>> stat) {
        ...
        private static <T> Codec<CriterionConditionPlayer.e<T>> createTypedCodec(StatisticWrapper<T> statisticwrapper) {
            return RecordCodecBuilder.create((instance) -> {
                return instance.group(statisticwrapper.getRegistry().holderByNameCodec().fieldOf("stat").forGetter(CriterionConditionPlayer.e::value), ExtraCodecs.strictOptionalField(CriterionConditionValue.IntegerRange.CODEC, "value", CriterionConditionValue.IntegerRange.ANY).forGetter(CriterionConditionPlayer.e::range)).apply(instance, (holder, criterionconditionvalue_integerrange) -> {
    ...
    private interface c extends Predicate<AdvancementProgress> {
        ...
        Codec<CriterionConditionPlayer.c> CODEC = Codec.either(CriterionConditionPlayer.b.CODEC, CriterionConditionPlayer.a.CODEC).xmap((either) -> {
            return (CriterionConditionPlayer.c) either.map((criterionconditionplayer_b) -> {
                return criterionconditionplayer_b;
            }, (criterionconditionplayer_a) -> {
                return criterionconditionplayer_a;
            });
        }, (criterionconditionplayer_c) -> {
            if (criterionconditionplayer_c instanceof CriterionConditionPlayer.b) {
                CriterionConditionPlayer.b criterionconditionplayer_b = (CriterionConditionPlayer.b) criterionconditionplayer_c;

                return Either.left(criterionconditionplayer_b);
            } else if (criterionconditionplayer_c instanceof CriterionConditionPlayer.a) {
                CriterionConditionPlayer.a criterionconditionplayer_a = (CriterionConditionPlayer.a) criterionconditionplayer_c;
    ...
    public static class d {
        ...
        private Optional<EnumGamemode> gameType;
