...
public class CommandAdvancement {
    ...
    private static int perform(CommandListenerWrapper commandlistenerwrapper, Collection<EntityPlayer> collection, CommandAdvancement.Action commandadvancement_action, Collection<AdvancementHolder> collection1) {
        int i = 0;
        ...
        if (i == 0) {
            if (collection1.size() == 1) {
                if (collection.size() == 1) {
                    throw new CommandException(IChatBaseComponent.translatable(commandadvancement_action.getKey() + ".one.to.one.failure", Advancement.name((AdvancementHolder) collection1.iterator().next()), ((EntityPlayer) collection.iterator().next()).getDisplayName()));
                } else {
                    throw new CommandException(IChatBaseComponent.translatable(commandadvancement_action.getKey() + ".one.to.many.failure", Advancement.name((AdvancementHolder) collection1.iterator().next()), collection.size()));
                }
    ...
    private static int performCriterion(CommandListenerWrapper commandlistenerwrapper, Collection<EntityPlayer> collection, CommandAdvancement.Action commandadvancement_action, AdvancementHolder advancementholder, String s) {
        int i = 0;
        Advancement advancement = advancementholder.value();

        if (!advancement.criteria().containsKey(s)) {
            throw new CommandException(IChatBaseComponent.translatable("commands.advancement.criterionNotFound", Advancement.name(advancementholder), s));
        } else {
            ...
            if (i == 0) {
                if (collection.size() == 1) {
                    throw new CommandException(IChatBaseComponent.translatable(commandadvancement_action.getKey() + ".criterion.to.one.failure", s, Advancement.name(advancementholder), ((EntityPlayer) collection.iterator().next()).getDisplayName()));
                } else {
                    throw new CommandException(IChatBaseComponent.translatable(commandadvancement_action.getKey() + ".criterion.to.many.failure", s, Advancement.name(advancementholder), collection.size()));
                }
            } else {
                if (collection.size() == 1) {
                    commandlistenerwrapper.sendSuccess(() -> {
