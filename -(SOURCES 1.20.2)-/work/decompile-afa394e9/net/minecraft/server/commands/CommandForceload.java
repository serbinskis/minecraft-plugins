...
public class CommandForceload {
    ...
    private static int queryForceLoad(CommandListenerWrapper commandlistenerwrapper, BlockPosition2D blockposition2d) throws CommandSyntaxException {
        ...
        if (flag) {
            commandlistenerwrapper.sendSuccess(() -> {
                return IChatBaseComponent.translatable("commands.forceload.query.success", chunkcoordintpair, resourcekey.location());
            }, false);
            return 1;
        } else {
    ...
    private static int listForceLoad(CommandListenerWrapper commandlistenerwrapper) {
        ...
        if (i > 0) {
            ...
            if (i == 1) {
                commandlistenerwrapper.sendSuccess(() -> {
                    return IChatBaseComponent.translatable("commands.forceload.list.single", resourcekey.location(), s);
                }, false);
            } else {
                commandlistenerwrapper.sendSuccess(() -> {
                    return IChatBaseComponent.translatable("commands.forceload.list.multiple", i, resourcekey.location(), s);
                }, false);
            }
        } else {
    ...
    private static int removeAll(CommandListenerWrapper commandlistenerwrapper) {
        ...
        commandlistenerwrapper.sendSuccess(() -> {
            return IChatBaseComponent.translatable("commands.forceload.removed.all", resourcekey.location());
        }, true);
        return 0;
    }
        ...
        if (i >= -30000000 && j >= -30000000 && k < 30000000 && l < 30000000) {
            ...
            if (i2 > 256L) {
                ...
                if (j2 == 0) {
                    ...
                    if (j2 == 1) {
                        commandlistenerwrapper.sendSuccess(() -> {
                            return IChatBaseComponent.translatable("commands.forceload." + (flag ? "added" : "removed") + ".single", chunkcoordintpair, resourcekey.location());
                        }, true);
                    } else {
                        ...
                        commandlistenerwrapper.sendSuccess(() -> {
                            return IChatBaseComponent.translatable("commands.forceload." + (flag ? "added" : "removed") + ".multiple", chunkcoordintpair, resourcekey.location(), chunkcoordintpair1, chunkcoordintpair2);
                        }, true);
                    }
