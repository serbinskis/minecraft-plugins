...
public class TagEntry {
    ...
    public static final Codec<TagEntry> CODEC = Codec.either(ExtraCodecs.TAG_OR_ELEMENT_ID, TagEntry.FULL_CODEC).xmap((either) -> {
        return (TagEntry) either.map((extracodecs_f) -> {
            return new TagEntry(extracodecs_f, true);
        }, (tagentry) -> {
            return tagentry;
        });
    }, (tagentry) -> {
    ...
    private TagEntry(ExtraCodecs.f extracodecs_f, boolean flag) {
        this.id = extracodecs_f.id();
        this.tag = extracodecs_f.tag();
        this.required = flag;
    }

    private ExtraCodecs.f elementOrTag() {
        return new ExtraCodecs.f(this.id, this.tag);
    }
