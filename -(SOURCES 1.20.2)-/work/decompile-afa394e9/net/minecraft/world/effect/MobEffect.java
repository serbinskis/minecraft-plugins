...
public class MobEffect implements Comparable<MobEffect> {
    ...
    public void onEffectStarted(EntityLiving entityliving) {
        this.effect.onEffectStarted(entityliving, this.amplifier);
    }
    ...
    public NBTTagCompound save(NBTTagCompound nbttagcompound) {
        MinecraftKey minecraftkey = BuiltInRegistries.MOB_EFFECT.getKey(this.effect);
    ...
    private void writeDetailsTo(NBTTagCompound nbttagcompound) {
        ...
        nbttagcompound.putBoolean("show_particles", this.isVisible());
        nbttagcompound.putBoolean("show_icon", this.showIcon());
        if (this.hiddenEffect != null) {
            NBTTagCompound nbttagcompound1 = new NBTTagCompound();

            this.hiddenEffect.save(nbttagcompound1);
            nbttagcompound.put("hidden_effect", nbttagcompound1);
        }
        ...
        this.factorData.ifPresent((mobeffect_a) -> {
            ...
            dataresult.resultOrPartial(logger::error).ifPresent((nbtbase) -> {
                nbttagcompound.put("factor_calculation_data", nbtbase);
            });
        });
    }
    ...
    public static MobEffect load(NBTTagCompound nbttagcompound) {
        ...
        MobEffectList mobeffectlist = (MobEffectList) BuiltInRegistries.MOB_EFFECT.get(MinecraftKey.tryParse(s));
    ...
    private static MobEffect loadSpecifiedEffect(MobEffectList mobeffectlist, NBTTagCompound nbttagcompound) {
        ...
        if (nbttagcompound.contains("show_particles", 1)) {
            flag1 = nbttagcompound.getBoolean("show_particles");
        }

        boolean flag2 = flag1;

        if (nbttagcompound.contains("show_icon", 1)) {
            flag2 = nbttagcompound.getBoolean("show_icon");
        }

        MobEffect mobeffect = null;

        if (nbttagcompound.contains("hidden_effect", 10)) {
            mobeffect = loadSpecifiedEffect(mobeffectlist, nbttagcompound.getCompound("hidden_effect"));
        }

        Optional optional;

        if (nbttagcompound.contains("factor_calculation_data", 10)) {
            DataResult dataresult = MobEffect.a.CODEC.parse(new Dynamic(DynamicOpsNBT.INSTANCE, nbttagcompound.getCompound("factor_calculation_data")));
            Logger logger = MobEffect.LOGGER;
