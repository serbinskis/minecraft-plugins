...
public class ArmorTrim {
    ...
    public static Optional<ArmorTrim> getTrim(IRegistryCustom iregistrycustom, ItemStack itemstack, boolean flag) {
        if (itemstack.is(TagsItem.TRIMMABLE_ARMOR) && itemstack.getTag() != null && itemstack.getTag().contains("Trim")) {
            ...
            ArmorTrim armortrim = (ArmorTrim) ArmorTrim.CODEC.parse(RegistryOps.create(DynamicOpsNBT.INSTANCE, (HolderLookup.b) iregistrycustom), nbttagcompound).resultOrPartial((s) -> {
                if (!flag) {
                    ArmorTrim.LOGGER.warn(s);
                }
    ...
    public static void appendUpgradeHoverText(ItemStack itemstack, IRegistryCustom iregistrycustom, List<IChatBaseComponent> list) {
        Optional<ArmorTrim> optional = getTrim(iregistrycustom, itemstack, true);
