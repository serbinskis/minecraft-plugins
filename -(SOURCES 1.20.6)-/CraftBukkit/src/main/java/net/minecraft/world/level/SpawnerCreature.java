...
public final class SpawnerCreature {
    ...
    private static boolean isValidSpawnPostitionForType(WorldServer worldserver, EnumCreatureType enumcreaturetype, StructureManager structuremanager, ChunkGenerator chunkgenerator, BiomeSettingsMobs.c biomesettingsmobs_c, BlockPosition.MutableBlockPosition blockposition_mutableblockposition, double d0) {
        ...
        return entitytypes.getCategory() == EnumCreatureType.MISC ? false : (!entitytypes.canSpawnFarFromPlayer() && d0 > (double) (entitytypes.getCategory().getDespawnDistance() * entitytypes.getCategory().getDespawnDistance()) ? false : (entitytypes.canSummon() && canSpawnMobAt(worldserver, structuremanager, chunkgenerator, enumcreaturetype, biomesettingsmobs_c, blockposition_mutableblockposition) ? (!EntityPositionTypes.isSpawnPositionOk(entitytypes, worldserver, blockposition_mutableblockposition) ? false : (!EntityPositionTypes.checkSpawnRules(entitytypes, worldserver, EnumMobSpawn.NATURAL, blockposition_mutableblockposition, worldserver.random) ? false : worldserver.noCollision(entitytypes.getSpawnAABB((double) blockposition_mutableblockposition.getX() + 0.5D, (double) blockposition_mutableblockposition.getY(), (double) blockposition_mutableblockposition.getZ() + 0.5D)))) : false));
    }
                    ...
                    for (int l1 = 0; l1 < k; ++l1) {
                        ...
                        for (int i2 = 0; !flag && i2 < 4; ++i2) {
                            ...
                            if (biomesettingsmobs_c.type.canSummon() && EntityPositionTypes.isSpawnPositionOk(biomesettingsmobs_c.type, worldaccess, blockposition)) {
                                ...
                                if (!worldaccess.noCollision(biomesettingsmobs_c.type.getSpawnAABB(d0, (double) blockposition.getY(), d1)) || !EntityPositionTypes.checkSpawnRules(biomesettingsmobs_c.type, worldaccess, EnumMobSpawn.CHUNK_GENERATION, BlockPosition.containing(d0, (double) blockposition.getY(), d1), worldaccess.getRandom())) {
                                    continue;
                                }
