...
public record ItemAttributeModifiersPredicate(Optional<CollectionPredicate<ItemAttributeModifiers.b, ItemAttributeModifiersPredicate.a>> modifiers) implements SingleComponentItemPredicate<ItemAttributeModifiers> {
    ...
    public static record a(Optional<HolderSet<AttributeBase>> attribute, Optional<UUID> id, Optional<String> name, CriterionConditionValue.DoubleRange amount, Optional<AttributeModifier.Operation> operation, Optional<EquipmentSlotGroup> slot) implements Predicate<ItemAttributeModifiers.b> {

        public static final Codec<ItemAttributeModifiersPredicate.a> CODEC = RecordCodecBuilder.create((instance) -> {
            return instance.group(RegistryCodecs.homogeneousList(Registries.ATTRIBUTE).optionalFieldOf("attribute").forGetter(ItemAttributeModifiersPredicate.a::attribute), UUIDUtil.LENIENT_CODEC.optionalFieldOf("uuid").forGetter(ItemAttributeModifiersPredicate.a::id), Codec.STRING.optionalFieldOf("name").forGetter(ItemAttributeModifiersPredicate.a::name), CriterionConditionValue.DoubleRange.CODEC.optionalFieldOf("amount", CriterionConditionValue.DoubleRange.ANY).forGetter(ItemAttributeModifiersPredicate.a::amount), AttributeModifier.Operation.CODEC.optionalFieldOf("operation").forGetter(ItemAttributeModifiersPredicate.a::operation), EquipmentSlotGroup.CODEC.optionalFieldOf("slot").forGetter(ItemAttributeModifiersPredicate.a::slot)).apply(instance, ItemAttributeModifiersPredicate.a::new);
        });

        public boolean test(ItemAttributeModifiers.b itemattributemodifiers_b) {
            return this.attribute.isPresent() && !((HolderSet) this.attribute.get()).contains(itemattributemodifiers_b.attribute()) ? false : (this.id.isPresent() && !((UUID) this.id.get()).equals(itemattributemodifiers_b.modifier().id()) ? false : (this.name.isPresent() && !((String) this.name.get()).equals(itemattributemodifiers_b.modifier().name()) ? false : (!this.amount.matches(itemattributemodifiers_b.modifier().amount()) ? false : (this.operation.isPresent() && this.operation.get() != itemattributemodifiers_b.modifier().operation() ? false : !this.slot.isPresent() || this.slot.get() == itemattributemodifiers_b.slot()))));
        }
    }
}
