...
public class ArgumentParserItemStack {
    ...
    public ArgumentParserItemStack.a parse(StringReader stringreader) throws CommandSyntaxException {
        ...
        final DataComponentMap.a datacomponentmap_a = DataComponentMap.builder();
        ...
        this.parse(stringreader, new ArgumentParserItemStack.d(this) {
            ...
            public <T> void visitComponent(DataComponentType<T> datacomponenttype, T t0) {
                datacomponentmap_a.set(datacomponenttype, t0);
            }
        });
        Holder<Item> holder = (Holder) Objects.requireNonNull((Holder) mutableobject.getValue(), "Parser gave no item");
        DataComponentMap datacomponentmap = datacomponentmap_a.build();

        validateComponents(stringreader, holder, datacomponentmap);
        return new ArgumentParserItemStack.a(holder, datacomponentmap);
    }
    ...
    private static void validateComponents(StringReader stringreader, Holder<Item> holder, DataComponentMap datacomponentmap) throws CommandSyntaxException {
        DataComponentMap datacomponentmap1 = DataComponentMap.composite(((Item) holder.value()).components(), datacomponentmap);
        DataResult<Unit> dataresult = ItemStack.validateComponents(datacomponentmap1);
    ...
    public static record a(Holder<Item> item, DataComponentMap components) {
    ...
    private class b {
        ...
        private CompletableFuture<Suggestions> suggestComponentAssignment(SuggestionsBuilder suggestionsbuilder) {
            ...
            ICompletionProvider.filterResources(BuiltInRegistries.DATA_COMPONENT_TYPE.entrySet(), s, (entry) -> {
