    ...
    public static GameTestHarnessRunner.b fromGameTestInfo() {
        return (collection) -> {
            ...
            return map.entrySet().stream().flatMap((entry) -> {
                ...
                return Streams.mapWithIndex(Lists.partition(list, 50).stream(), (list1, i) -> {
                    return toGameTestBatch(List.copyOf(list1), s, i);
                });
            }).toList();
        };
    }

    private static GameTestHarnessBatch toGameTestBatch(List<GameTestHarnessInfo> list, String s, long i) {
        ...
        return new GameTestHarnessBatch(s + ":" + i, list, consumer, consumer1);
    }
}
