...
public class TextComponentTagVisitor implements TagVisitor {
    ...
    private TextComponentTagVisitor(String s, int i, int j) {
    ...
    public void visitList(NBTTagList nbttaglist) {
        if (nbttaglist.isEmpty()) {
            ...
            this.result = IChatBaseComponent.literal("[").append(TextComponentTagVisitor.TOO_DEEP).append("]");
        } else {
            int i;

            if (TextComponentTagVisitor.INLINE_ELEMENT_TYPES.contains(nbttaglist.getElementType()) && nbttaglist.size() <= 8) {
                ...
                for (i = 0; i < nbttaglist.size(); ++i) {
                    ...
                    ichatmutablecomponent.append(this.buildSubTag(nbttaglist.get(i), false));
                }
                ...
                for (i = 0; i < nbttaglist.size(); ++i) {
                    IChatMutableComponent ichatmutablecomponent2 = IChatBaseComponent.literal(s1);

                    ichatmutablecomponent2.append(this.buildSubTag(nbttaglist.get(i), true));
    ...
    public void visitCompound(NBTTagCompound nbttagcompound) {
        if (nbttagcompound.isEmpty()) {
            ...
            for (Iterator<String> iterator = ((Collection) collection).iterator(); iterator.hasNext(); ichatmutablecomponent.append((IChatBaseComponent) ichatmutablecomponent1)) {
                String s1 = (String) iterator.next();

                ichatmutablecomponent1 = IChatBaseComponent.literal(s).append(handleEscapePretty(s1)).append(TextComponentTagVisitor.NAME_VALUE_SEPARATOR).append(" ").append(this.buildSubTag(nbttagcompound.get(s1), true));
                if (iterator.hasNext()) {
    ...
    private IChatBaseComponent buildSubTag(NBTBase nbtbase, boolean flag) {
        return (new TextComponentTagVisitor(this.indentation, flag ? this.indentDepth + 1 : this.indentDepth, this.depth + 1)).visit(nbtbase);
    }
