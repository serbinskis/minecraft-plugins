...
public class PlayerInventory implements IInventory, INamableTileEntity {
    ...
    public int findSlotMatchingUnusedItem(ItemStack itemstack) {
        for (int i = 0; i < this.items.size(); ++i) {
            ...
            if (!itemstack1.isEmpty() && ItemStack.isSameItemSameComponents(itemstack, itemstack1) && !itemstack1.isDamaged() && !itemstack1.isEnchanted() && !itemstack1.has(DataComponents.CUSTOM_NAME)) {
                return i;
            }
        }
