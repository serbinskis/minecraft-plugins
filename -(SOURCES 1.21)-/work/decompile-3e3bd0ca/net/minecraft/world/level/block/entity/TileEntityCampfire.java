...
public class TileEntityCampfire extends TileEntity implements Clearable {
    ...
    private final CraftingManager.a<SingleRecipeInput, RecipeCampfire> quickCheck;
        ...
        for (int i = 0; i < tileentitycampfire.items.size(); ++i) {
            ...
            if (!itemstack.isEmpty()) {
                ...
                if (tileentitycampfire.cookingProgress[i] >= tileentitycampfire.cookingTime[i]) {
                    ...
                    ItemStack itemstack1 = (ItemStack) tileentitycampfire.quickCheck.getRecipeFor(singlerecipeinput, world).map((recipeholder) -> {
                        return ((RecipeCampfire) recipeholder.value()).assemble(singlerecipeinput, world.registryAccess());
                    }).orElse(itemstack);
                    ...
                    if (itemstack1.isItemEnabled(world.enabledFeatures())) {
                        ...
                        world.gameEvent((Holder) GameEvent.BLOCK_CHANGE, blockposition, GameEvent.a.of(iblockdata));
                    }
                }
            }
        }
    ...
    public boolean placeFood(@Nullable EntityLiving entityliving, ItemStack itemstack, int i) {
