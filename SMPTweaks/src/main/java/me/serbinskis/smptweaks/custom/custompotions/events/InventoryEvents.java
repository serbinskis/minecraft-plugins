...
public class InventoryEvents implements Listener {
	...
	public void onInventoryClick(InventoryClickEvent event) {
		...
		if (event.getInventory() instanceof BrewerInventory) {
			CustomPotion customPotion = PotionManager.getCustomPotion(event.getCurrentItem());
		...
		TaskUtils.scheduleSyncDelayedTask(() -> {
			...
			for (int i = 0; i < inv.getSize(); i++) {
				if (PotionManager.getCustomPotion(inv.getItem(i)) == null) { continue; }
	...
	public void onInventoryDrag(InventoryDragEvent event) {
		...
		if (event.getInventory() instanceof BrewerInventory inv) {
			TaskUtils.scheduleSyncDelayedTask(() -> {
				for (int i = 0; i < inv.getSize(); i++) {
					CustomPotion customPotion = PotionManager.getCustomPotion(inv.getItem(i));
